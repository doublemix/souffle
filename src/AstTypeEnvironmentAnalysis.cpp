...
void TypeEnvironmentAnalysis::updateTypeEnvironment(const AstProgram& program) {
	...
	for (const auto& cur : program.getTypes()) {
		...
		} else if (dynamic_cast<const AstEnumType*>(cur)) {
			// initialize the enum
			env.createEnumType(cur->getName());
		} else {
	...
	for (const auto& cur : program.getTypes()) {
		...
		} else if (auto* t = dynamic_cast<const AstEnumType*>(cur)) {
			auto* et = dynamic_cast<EnumType*>(type);
			if (!et) {
				continue;  // support faulty input
			}

			// add cases
			for (const auto& c : t->getCases()) {
				if (env.isType(c.type)) {
					et->add(c.name, env.getType(c.type));
				}
			}
