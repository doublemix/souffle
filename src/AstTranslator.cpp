...
std::unique_ptr<RamLatticeGLB> AstTranslator::makeRamLatticeGLB(const std::set<Location>& locs) {
	auto res = std::make_unique<RamLatticeGLB>();
	for (const auto& loc : locs) {
		res->addRef(loc.identifier, loc.element, std::unique_ptr<RamRelationReference>(loc.relation->clone()));
	}
	return std::move(res);
}
...
std::unique_ptr<RamRelationReference> AstTranslator::createRelationReference(const std::string name,
	...
	if (ramRel == nullptr) {
		ramProg->addRelation(std::make_unique<RamRelation>(
				name, arity, attributeNames, attributeTypeQualifiers, mask, representation, latticeFlag));
...
std::unique_ptr<RamRelationReference> AstTranslator::translateRelation(
	...
	return createRelationReference(relationNamePrefix + getRelationName(rel->getName()), rel->getArity(),
			attributeNames, attributeTypeQualifiers, getSymbolMask(*rel), rel->getRepresentation(), rel->isLattice());
}
...
std::unique_ptr<RamValue> AstTranslator::translateValue(const AstArgument* arg, const ValueIndex& index) {
	...
	: translator(translator), index(index) {}
		...
		std::unique_ptr<RamValue> visitVariable(const AstVariable& var) override {
			...
			if (index.isArgEnum(index.getDefinitionPoint(var), translator.typeEnv)) {
				std::cout << "Enum type detected! all loc size:" << index.getVariableReferences().at(var.getName()).size() << "\n";
				if (index.getVariableReferences().at(var.getName()).size() == 1)
					return makeRamElementAccess(index.getDefinitionPoint(var));
				else
					return makeRamLatticeGLB( index.getVariableReferences().at(var.getName()) );
			}
