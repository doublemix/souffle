...
bool AstSemanticChecker::transform(AstTranslationUnit& translationUnit) {
	const TypeEnvironment& typeEnv = translationUnit.getAnalysis<
			TypeEnvironmentAnalysis>()->getTypeEnvironment();
	...
	checkProgram(translationUnit.getErrorReport(),
			*translationUnit.getProgram(), typeEnv, *typeAnalysis,
			*precedenceGraph, *recursiveClauses, *ioTypes);
	return false;
}

void AstSemanticChecker::checkProgram(ErrorReport& report,
	...
	if (Global::config().has("suppress-warnings")) {
		std::vector<std::string> suppressedRelations = splitString(
				Global::config().get("suppress-warnings"), ',');
			...
			for (auto& relname : suppressedRelations) {
				...
				if (!comps.empty()) {
					...
					if (AstRelation* rel = program.getRelation(relid)) {
						rel->setQualifier(
								rel->getQualifier() | SUPPRESSED_RELATION);
					}
				}
			}
		}
	...
	visitDepthFirst(nodes,
			[&](const AstUserDefinedFunctor& fun) {
	...
	const Graph<const AstRelation*, AstNameComparison>& depGraph =
	...
	for (const AstRelation* cur : depGraph.vertices()) {
		if (depGraph.reaches(cur, cur)) {
			...
			for (const AstRelation* cyclicRelation : clique) {
				...
				bool hasNegation = hasClauseWithNegatedRelation(cyclicRelation,
						cur, &program, foundLiteral);
				if (hasNegation
						|| hasClauseWithAggregatedRelation(cyclicRelation, cur,
								&program, foundLiteral)) {
					std::string relationsListStr =
							toString(
									join(clique, ",",
					...
					messages.push_back(
							DiagnosticMessage(
									"Relation " + toString(cur->getName()),
									cur->getSrcLoc()));
					std::string negOrAgg =
							hasNegation ? "negation" : "aggregation";
					messages.push_back(
							DiagnosticMessage("has cyclic " + negOrAgg,
									foundLiteral->getSrcLoc()));
...
void AstSemanticChecker::checkAtom(ErrorReport& report,
		const AstProgram& program, const AstAtom& atom) {
	// check existence of relation
	auto* r = program.getRelation(atom.getName());
	if (!r) {
		report.addError("Undefined relation " + toString(atom.getName()),
				atom.getSrcLoc());
	}
	...
	if (r && r->getArity() != atom.getArity()) {
		report.addError(
				"Mismatching arity of relation " + toString(atom.getName()),
				atom.getSrcLoc());
	}
...
static bool hasUnnamedVariable(const AstArgument* arg) {
	...
	if (const auto* inf = dynamic_cast<const AstIntrinsicFunctor*>(arg)) {
		return any_of(inf->getArguments(),
				(bool (*)(const AstArgument*))hasUnnamedVariable);
	}
	if (const auto* udf = dynamic_cast<const AstUserDefinedFunctor*>(arg)) {
		return any_of(udf->getArguments(),
				(bool (*)(const AstArgument*))hasUnnamedVariable);
	}
	if (const auto* udf = dynamic_cast<const AstLatticeUnaryFunctor*>(arg)) {
		return hasUnnamedVariable(udf->getArgument());
	}
	if (const auto* udf = dynamic_cast<const AstLatticeBinaryFunctor*>(arg)) {
		return any_of(udf->getArguments(),
				(bool (*)(const AstArgument*))hasUnnamedVariable);
	}
...
void AstSemanticChecker::checkLiteral(ErrorReport& report,
		const AstProgram& program, const AstLiteral& literal) {
	...
	if (const auto* constraint =
			dynamic_cast<const AstBinaryConstraint*>(&literal)) {
	...
	if (hasUnnamedVariable(&literal)) {
		if (dynamic_cast<const AstAtom*>(&literal)) {
			...
			report.addError("Underscore in binary relation",
					literal.getSrcLoc());
		} else {
...
void AstSemanticChecker::checkAggregator(ErrorReport& report,
		const AstProgram& program, const AstAggregator& aggregator) {
	for (AstLiteral* literal : aggregator.getBodyLiterals()) {
		checkLiteral(report, program, *literal);
	}
}

void AstSemanticChecker::checkArgument(ErrorReport& report,
		const AstProgram& program, const AstArgument& arg) {
	if (const auto* agg = dynamic_cast<const AstAggregator*>(&arg)) {
		checkAggregator(report, program, *agg);
	} else if (const auto* intrFunc =
			dynamic_cast<const AstIntrinsicFunctor*>(&arg)) {
		for (size_t i = 0; i < intrFunc->getArity(); i++) {
			checkArgument(report, program, *intrFunc->getArg(i));
		}
	} else if (const auto* userDefFunc =
			dynamic_cast<const AstUserDefinedFunctor*>(&arg)) {
...
void AstSemanticChecker::checkConstant(ErrorReport& report,
		const AstArgument& argument) {
	if (const auto* var = dynamic_cast<const AstVariable*>(&argument)) {
		report.addError("Variable " + var->getName() + " in fact",
				var->getSrcLoc());
...
void AstSemanticChecker::checkFact(ErrorReport& report,
		const AstProgram& program, const AstClause& fact) {
	assert(fact.isFact());
...
void AstSemanticChecker::checkClause(ErrorReport& report,
		const AstProgram& program, const AstClause& clause,
	...
	if (hasUnnamedVariable(clause.getHead())) {
		report.addError("Underscore in head of rule",
				clause.getHead()->getSrcLoc());
	}
	...
	if (!clause.isGenerated()) {
		for (const auto& cur : var_count) {
			if (cur.second == 1 && cur.first[0] != '_') {
				report.addWarning("Variable " + cur.first + " only occurs once",
						var_pos[cur.first]->getSrcLoc());
			}
		}
	}
	...
	if (clause.getExecutionPlan()) {
		...
		for (const auto& cur : clause.getExecutionPlan()->getOrders()) {
			if (cur.second->size() != numAtoms || !cur.second->isComplete()) {
				report.addError("Invalid execution plan",
	...
	if (recursiveClauses.recursive(&clause)) {
		visitDepthFirst(clause,
				[&](const AstCounter& ctr) {
					report.addError("Auto-increment functor in a recursive rule", ctr.getSrcLoc());
				});
	}
}

void AstSemanticChecker::checkRelationDeclaration(ErrorReport& report,
		const TypeEnvironment& typeEnv, const AstProgram& program,
		const AstRelation& relation, const IOType& ioTypes) {
	for (size_t i = 0; i < relation.getArity(); i++) {
		...
		if (typeName != "number" && typeName != "symbol"
				&& !program.getType(typeName)) {
			report.addError(
					"Undefined type in attribute " + attr->getAttributeName()
		...
		for (size_t j = 0; j < i; j++) {
			if (attr->getAttributeName()
					== relation.getAttribute(j)->getAttributeName()) {
				report.addError(
...
void AstSemanticChecker::checkRelation(ErrorReport& report,
		const TypeEnvironment& typeEnv, const AstProgram& program,
	...
	if (relation.getRepresentation() == RelationRepresentation::EQREL) {
		if (relation.getArity() == 2) {
			if (relation.getAttribute(0)->getTypeName()
				...
				report.addError(
						"Domains of equivalence relation "
								+ toString(relation.getName())
								+ " are different", relation.getSrcLoc());
			}
		} else {
			report.addError(
					"Equivalence relation " + toString(relation.getName())
							+ " is not binary", relation.getSrcLoc());
		}
	}
	...
	if (relation.clauseSize() == 0 && !ioTypes.isInput(&relation)
			&& !relation.isSuppressed()) {
		report.addWarning(
				"No rules/facts defined for relation "
						+ toString(relation.getName()), relation.getSrcLoc());
	}
}

void AstSemanticChecker::checkRules(ErrorReport& report,
		const TypeEnvironment& typeEnv, const AstProgram& program,
		const RecursiveClauses& recursiveClauses, const IOType& ioTypes) {
...
static bool unionContainsNumber(const AstProgram& program,
		const AstUnionType& type) {
	...
	for (const AstTypeIdentifier& elemTypeID : type.getTypes()) {
		...
		if (const auto* primitive =
				dynamic_cast<const AstPrimitiveType*>(elemType)) {
...
static bool unionContainsSymbol(const AstProgram& program,
		const AstUnionType& type) {
...
void AstSemanticChecker::checkUnionType(ErrorReport& report,
	...
	for (const AstTypeIdentifier& sub : type.getTypes()) {
		if (sub != "number" && sub != "symbol") {
			...
			if (!subt) {
				report.addError(
						"Undefined type " + toString(sub)
								+ " in definition of union type "
								+ toString(type.getName()), type.getSrcLoc());
			} else if (!dynamic_cast<const AstUnionType*>(subt)
					&& !dynamic_cast<const AstPrimitiveType*>(subt)) {
				report.addError(
						"Union type " + toString(type.getName())
								+ " contains the non-primitive type "
								+ toString(sub), type.getSrcLoc());
			}
		}
	}

	// check all element types are based on the same primitive
	if (unionContainsSymbol(program, type)
			&& unionContainsNumber(program, type)) {
		report.addError(
				"Union type " + toString(type.getName())
...
void AstSemanticChecker::checkRecordType(ErrorReport& report,
		const AstProgram& program, const AstRecordType& type) {
	// check proper definition of all field types
	for (const auto& field : type.getFields()) {
		if (field.type != "number" && field.type != "symbol"
				&& !program.getType(field.type)) {
			report.addError(
					"Undefined type " + toString(field.type)
							+ " in definition of field " + field.name,
					type.getSrcLoc());
		}
	}
	...
	for (std::size_t i = 0; i < numFields; i++) {
		...
		for (std::size_t j = 0; j < i; j++) {
			if (fields[j].name == cur_name) {
				report.addError(
						"Doubly defined field name " + cur_name
...
void AstSemanticChecker::checkType(ErrorReport& report,
...
void AstSemanticChecker::checkTypes(ErrorReport& report,
...
void AstSemanticChecker::checkIODirectives(ErrorReport& report,
		const AstProgram& program) {
...
static const std::vector<SrcLocation> usesInvalidWitness(
	...
	for (const AstLiteral* lit : literals) {
		...
		visitDepthFirst(*firstClone,
				[&](const AstArgument& arg) {firstCloneArguments.push_back(&arg);});

		std::vector<const AstArgument*> secondCloneArguments;
		visitDepthFirst(*secondClone,
				[&](const AstArgument& arg) {secondCloneArguments.push_back(&arg);});

		for (size_t i = 0; i < firstCloneArguments.size(); i++) {
			identicalSubnodeMap[secondCloneArguments[i]] =
					firstCloneArguments[i];
		}
	...
	auto groundingAtomAggregatorless = std::make_unique<AstAtom>(
			"grounding_atom");
	...
	const std::set<std::string>& aggregatorVariables =
			update.getAggregatorVariables();
	for (const std::string& str : aggregatorVariables) {
		groundingAtomAggregatorless->addArgument(
				std::make_unique<AstVariable>(str));
	}
	...
	for (const std::unique_ptr<AstArgument>& arg : groundedArguments) {
		...
		groundingAtomOriginal->addArgument(
				std::unique_ptr<AstArgument>(arg->clone()));
	}
	...
	std::map<const AstArgument*, bool> originalGrounded = getGroundedTerms(
			*originalClause);
	std::map<const AstArgument*, bool> aggregatorlessGrounded =
			getGroundedTerms(*aggregatorlessClause);
...
void AstSemanticChecker::checkWitnessProblem(ErrorReport& report,
		const AstProgram& program) {
 ...
 */
std::vector<AstRelationIdentifier> findInlineCycle(
		const PrecedenceGraph& precedenceGraph,
		std::map<const AstRelation*, const AstRelation*>& origins,
		const AstRelation* current, AstRelationSet& unvisited,
		AstRelationSet& visiting, AstRelationSet& visited) {
	...
	if (current == nullptr) {
		...
		std::vector<AstRelationIdentifier> subresult = findInlineCycle(
				precedenceGraph, origins, current, unvisited, visiting,
				visited);

		if (subresult.empty()) {
			// No cycle found, try again from another node
			return findInlineCycle(precedenceGraph, origins, nullptr, unvisited,
					visiting, visited);
		} else {
	...
	// Check neighbours
	const AstRelationSet& successors = precedenceGraph.graph().successors(
			current);
	for (const AstRelation* successor : successors) {
		...
		if (successor->isInline()) {
			...
			std::vector<AstRelationIdentifier> subgraphCycle = findInlineCycle(
					precedenceGraph, origins, successor, unvisited, visiting,
					visited);
...
void AstSemanticChecker::checkInlining(ErrorReport& report,
		const AstProgram& program, const PrecedenceGraph& precedenceGraph,
		const IOType& ioTypes) {
	...
	for (const auto& relation : program.getRelations()) {
		if (relation->isInline()) {
			...
			if (ioTypes.isIO(relation)) {
				report.addError(
						"IO relation " + toString(relation->getName())
								+ " cannot be inlined", relation->getSrcLoc());
			}
		}
	}
	...
	std::vector<AstRelationIdentifier> result = findInlineCycle(precedenceGraph,
			origins, nullptr, unvisited, visiting, visited);

	// If the result contains anything, then a cycle was found
	if (!result.empty()) {
		AstRelation* cycleOrigin = program.getRelation(
		...
		report.addError(
				"Cannot inline cyclically dependent relations " + cycle.str(),
				cycleOrigin->getSrcLoc());
	}
	...
	for (const AstRelation* rel : inlinedRelations) {
		...
		for (const AstClause* clause : rel->getClauses()) {
			...
			visitDepthFirst(*clause->getHead(),
					[&](const AstVariable& var) {headVariables.insert(var.getName());});
...
void AstSemanticChecker::checkNamespaces(ErrorReport& report,
		const AstProgram& program) {
