...
;// -- Table: a
std::unique_ptr<t_btree_1__0__1> rel_1_a = std::make_unique<t_btree_1__0__1>();
souffle::RelationWrapper<0,t_btree_1__0__1,Tuple<RamDomain,1>,1> wrapper_rel_1_a;
// -- Table: @delta_a
std::unique_ptr<t_btree_1__0> rel_2_delta_a = std::make_unique<t_btree_1__0>();
// -- Table: @new_a
std::unique_ptr<t_btree_1__0> rel_3_new_a = std::make_unique<t_btree_1__0>();
// -- Table: b
std::unique_ptr<t_btree_1__0> rel_4_b = std::make_unique<t_btree_1__0>();
souffle::RelationWrapper<1,t_btree_1__0,Tuple<RamDomain,1>,1> wrapper_rel_4_b;
public:
Sf_mitch() : 
wrapper_rel_1_a(*rel_1_a,symTable,"a",std::array<const char *,1>{{"i:number"}},std::array<const char *,1>{{"n"}}),

wrapper_rel_4_b(*rel_4_b,symTable,"b",std::array<const char *,1>{{"i:number"}},std::array<const char *,1>{{"n"}}){
...
/* BEGIN STRATUM 0 */
[&]() {
SignalHandler::instance()->setMsg(R"_(a(1).
in file /home/doublemix/workspace/souffle-qxg2/mitch/test4.dl [4:1-4:6])_");
rel_1_a->insert(RamDomain(1));
rel_2_delta_a->insertAll(*rel_1_a);
iter = 0;
for(;;) {
SignalHandler::instance()->setMsg(R"_(a(Intrsc(n + 1)) :- 
   a(n),
   n < 100.
in file /home/doublemix/workspace/souffle-qxg2/mitch/test4.dl [5:1-5:27])_");
...
if( ((env0[0]) < (RamDomain(100)))) {
if( !(rel_1_a->contains(Tuple<RamDomain,1>({{(env0[0]) + (RamDomain(1))}}),READ_OP_CONTEXT(rel_1_a_op_ctxt)))) {
...
/* BEGIN STRATUM 1 */
[&]() {
SignalHandler::instance()->setMsg(R"_(b(Intrsc(n * 2)) :- 
   a(n).
in file /home/doublemix/workspace/souffle-qxg2/mitch/test4.dl [7:1-7:18])_");
if (!rel_1_a->empty()) [&](){
...
pfor(auto it = part.begin(); it<part.end();++it){
try{for(const auto& env0 : *it) {
Tuple<RamDomain,1> tuple({{static_cast<RamDomain>((env0[0]) * (RamDomain(2)))}});
...
();if (performIO) {
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","n"},{"filename","mitch/out//b.csv"},{"name","b"}});
if (!outputDirectory.empty() && directiveMap["IO"] == "file" && directiveMap["filename"].front() != '/') {directiveMap["filename"] = outputDirectory + "/" + directiveMap["filename"];}
IODirectives ioDirectives(directiveMap);
IOSystem::getInstance().getWriter(SymbolMask({0}), EnumTypeMask({0}), symTable, ioDirectives, 0)->writeAll(*rel_4_b);
...
{
try{
souffle::CmdOptions opt(R"(mitch/test4.dl)",
R"(.)",
R"(.)",
false,
R"()",
